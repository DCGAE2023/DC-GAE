...
class Model(nn.Module):
    ...
    def forward(self, data, epoch):
        ...
        data.mask_weight = self.mask_weight_calc(data)
        mask_mode = 0 
        compose = 0 
        extra_loss = True 
    ...
    def classify_batch(self, loader):
        
        # Initialize a dictionary to store the embeddings of all nodes in the graph
        embeddings = {}
        for batch in loader:
            x, edge_index = batch.x, batch.edge_index
            x = self.shared_encoder(x, edge_index)
            # Update the node embedding of the current batch into the dictionary. If there are overlapping nodes, overwrite the old embedding.
            for i in range(len(x)):
                node_id = batch.n_id[i].item()
                embeddings[node_id] = x[i] 

        # Finally, convert the dictionary into a list, arranged in order of node numbers
    ...
    def threshold_loss(self, adj, A_hat, attrs, X_hat, epoch):
        ...
        if(False):
            ...
            loss = (1 - beta) * huber_loss(attrs, X_hat) + beta * huber_loss(adj, A_hat)
